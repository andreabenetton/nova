...
- `python3 tools/check_licenses.py` — 1,019 files classified, 292 directory markers present, and all Rust crate license fields matched policy;
- `python3 tools/ci/check_dco.py HEAD` — DCO checker executed successfully against an empty comparison range;
...
The licensing change also passed SPDX-header validation, license-text presence
checks, per-directory marker validation, Cargo package metadata validation,
Python byte-code compilation for the new licensing and DCO tools, repository
shape validation, contract-version immutability checks, and `git diff --check`.
The DCO workflow applies to commits introduced by future pull requests; earlier
scaffold history was not rewritten.
